...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    setLeagues(state, leagues) {
      state.leagues = leagues.sort(
        (a, b) => b.profiles_count - a.profiles_count
      );
    },
  ...
  actions: {
    ...
    signup(
      { commit },
      { email, firstName, lastName, password, squadName, favoriteTeam }
    ) {
      return api
        .signup(email, firstName, lastName, password, squadName, favoriteTeam)
        .then(token => {
          commit("setToken", token);
        });
    },
    ...
    getGameweeks({ commit }) {
      return api.getGameweeks().then(gameweeks => {
        commit("setGameweeks", gameweeks);
      });
    },

    getLeagues({ commit }) {
      return api.getLeagues().then(leagues => {
  ...
  getters: {
    ...
    activeGameweek(state) {
      return state.gameweeks.find(gameweek => gameweek.is_active === true);
    },

    nextGameweek(state) {
      return state.gameweeks.find(gameweek => gameweek.is_next === true);
    },

    goalkeepers(state) {
      return state.players.filter(
        player => player.position.short_name === "GK"
      );
    },
    ...
    unavailablePlayers(state) {
      return state.players.filter(player => player.availability !== null);
    }
  }
});
