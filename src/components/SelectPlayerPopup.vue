<template>
  <v-dialog v-model="popupOpen" max-width="600">
    <template v-slot:activator="{ on }">
      <v-icon
        :disabled="isBlankPlayer"
        color="red"
        class="warning"
        small
        right
      ...
      <v-layout justify-center>
        <v-img
          :src="!isBlankPlayer ? player.team.shirt : '/images/blank.png'"
      ...
      <div class="darkGreen">
        <v-layout justify-center>
          <span
            :class="{ 'hide-text': isBlankPlayer }"
            class="font-weight-regular white--text text-xs-center"
            >{{ !isBlankPlayer ? player.last_name : "position" }}</span
          >
        </v-layout>
        <hr />
        <v-layout justify-center>
          <span
            ...
            >{{ !isBlankPlayer ? player.price : "price" }}</span
          >
        </v-layout>
      </div>
    </template>
      ...
      <v-card-text>
        ...
        >
          <template v-slot:items="props">
            <tr>
              <td class="no-padding">
                <v-btn icon small @click="addPlayer(props.item)"
              ...
              <td nowrap>
                ...
                <v-tooltip v-if="!props.item.availability" right>
                  <template v-slot:activator="{ on }">
                    <v-icon small color="green" v-on="on">check_circle</v-icon>
                  </template>
                  <span>Available</span>
                </v-tooltip>
              ...
              <td>
                <v-img max-width="35" :src="props.item.team.shirt"></v-img>
              </td>
            </tr>
          </template>
        </v-data-table>
  ...
  methods: {
    ...
    filterNames(players, search, filter) {
      ...
      return players.filter(player => {
        return (
          filter(player.last_name, search) ||
