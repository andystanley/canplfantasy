<template>
  <v-dialog max-width="600" v-model="popupOpen">
    <template v-slot:activator="{ on }">
        ...
        <div class="darkGreen">
          <v-layout justify-center>
            <span :class="{'hide-text': !player}" class="font-weight-regular white--text text-xs-center">{{ player ? player.last_name : 'position' }}</span>
          </v-layout>
          <hr>
          <v-layout justify-center>
            <span :class="{'hide-text': !player}" class="font-weight-medium white--text">{{ player ? player.price : 'price' }}</span>
          </v-layout>
        </div>
    </template>
    <v-card>
